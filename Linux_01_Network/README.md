...
A port is given to an application running on some network host to communicate with applications running on other network hosts (including other applications on the same host). In software terms, a port can be thought of as an identifier that is bound to a data buffer managed by a service through the OS kernel.
...
- Install **Ubuntu 24.04 Server LTS** on all virtual machines created during the task.
